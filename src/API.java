...
public class API {
	...
	// Randomizer helper
	public void randomizer() throws FileNotFoundException {
		ArrayList<String> files = selectDifficulty();
		randomizer(files);
	}
	// Recursively selects a random quiz to run
	private void randomizer(ArrayList<String> files) {
	...
	private ArrayList<String> selectDifficulty() throws FileNotFoundException {
		...
		// This is a ugly hack
		Scanner s = new Scanner(new File(difficulty+".files"));
		while (s.hasNext()) {
			tmp.add(s.next());
		}
		s.close();
